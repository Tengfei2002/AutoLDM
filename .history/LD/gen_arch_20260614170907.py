...
def export_to_obj(blocks: List[Block], output_dir: str):
    """
    将几何块导出为标准的 Wavefront .obj 三维模型文件。
    每个包络盒（Bounding Box）将被解析为 8 个顶点和 12 个三角面。
    """
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        vertex_offset = 1  # OBJ 文件的顶点索引从 1 开始
        ...
        for block in blocks:
            ...
            # 定义长方体的 6 个面（每个面由 2 个三角形组成，确保兼容性）
            # 索引依据上方的 vertices 列表顺序，并加上当前的 vertex_offset
            faces = [
                # 底面 (z_min)
                (1, 4, 3), (1, 3, 2),
                # 顶面 (z_max)
...
if __name__ == "__main__":
    ...
    # 3. 导出 JSON 数据至 ./output/
    export_to_json(cfet_mesh, './output')
    
    # 4. 导出 3D 可视化文件至 ./output/
