生成过程参考的文件：
- 版图文件：./layout/xxx_layout.txt
- 规则文件：./rules/xxx_arch.txt
...
- 生成High-k介质，High-k的位置在channel的沿 y、z 方向环绕，前后上下的环绕厚度由 ./rules/xxx_arch.txt中的 high_k_thickness决定
  - high-k层为HfO2材料，其位置描述如下：High-k层作为一个厚度为 $t_{hk}$ 的保形介质层，构成了栅极金属在三维空间中的绝对外壳。它将栅极金属完全包络于其内侧体积中，作为栅极金属与外部结构的唯一交界面。在有效栅长（$L_g$）范围内，High-k层沿 $x$ 轴延伸，并在 $y-z$ 平面上正交闭合并紧密包裹住每一层悬浮的半导体硅纳米片沟道。在 $z$ 方向上相邻的纳米片间隙处，High-k层向 $x$ 轴的正负两端延伸，直到与内侧墙（Inner Spacer）直接接触，形成垂直于 $x$ 轴的物理截面。内侧墙在此充当了关键的空间阻挡层，确保High-k层与源漏外延材料（S/D Epi）保持绝对的物理隔离，使得该区域沿 $x$ 轴的材料相交排布严格遵循“栅极金属、High-k层、内侧墙、源漏外延”的拓扑序列。
- 生成内侧墙，内侧墙的位置在high-k的外侧，厚度为 ./rules/xxx_arch.txt中的 inner_spacer_thickness，整体形貌是分布在high-k两侧的两个窄长方体，长方体被沟道破开；材料为Si3N4材料
- 生成源漏外延，源漏外延的位置紧邻由内侧墙和沟道组成的平面，x方向上占据所有在boundary中未被沟道、栅极、内侧墙、high-k占据的区域，上下两个区域分别在其两侧各形成一个源漏外延，上层或下层的若干个沟道共享一个源漏外延，上层和下层的沟道源漏外延分离；以上层的源漏外延举例，y和z方向上，源漏外延在沿x的视角需要完整覆盖沟道的截面，现在将几条纳米片的边界框在最小的矩形中，源漏外延的大小在这个矩形的基础上分别大了sd_overgrowth_y(向两侧各sd_overgrowth_y),sd_overgrowth_z_up,sd_overgrowth_z_down，若这几个量中存在-1，则依靠sd_upper_overgrowth_y, sd_lower_overgrowth_y, sd_upper_overgrowth_z_up, sd_upper_overgrowth_z_down, sd_lower_overgrowth_z_up, sd_lower_overgrowth_z_down决定对应的值；