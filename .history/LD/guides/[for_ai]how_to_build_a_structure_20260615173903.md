...
- 生成源漏外延，源漏外延的位置紧邻由内侧墙和沟道组成的平面，x方向上占据所有在boundary中未被沟道、栅极、内侧墙、high-k占据的区域，上下两个区域分别在其两侧各形成一个源漏外延，上层或下层的若干个沟道共享一个源漏外延，上层和下层的沟道源漏外延分离；以上层的源漏外延举例，y和z方向上，源漏外延在沿x的视角需要完整覆盖沟道的截面，现在将几条纳米片的边界框在最小的矩形中，源漏外延的大小在这个矩形的基础上分别大了sd_overgrowth_y(向两侧各sd_overgrowth_y),sd_overgrowth_z_up,sd_overgrowth_z_down，若这几个量中存在-1，则

sd_upper_overgrowth_y = 0.005
sd_lower_overgrowth_y = 0.005
sd_upper_overgrowth_z_up = 0.006
sd_upper_overgrowth_z_down = 0.006
sd_lower_overgrowth_z_up = 0.006
sd_lower_overgrowth_z_down = 0.006